...
const NFTCard: React.FC<NFTCardProps & { index: number }> = ({
  ...
  const getMobieClasses = (index: number) => {
    let classes = "absolute sm:relative sm:rotate-0 sm:left-0 sm:right-0 sm:translate-x-0 ";
  ...
  return (
    <div
      className={`card hover:flipped lg:h-[311.24px] lg:w-[222.09px] w-[100px] h-[143px] ${getMobieClasses(index)}`}
    >
      <div className="card-inner">
        ...
        <div className={`card-back ${color}`}>
          <div className="p-2 md:p-6 -rotate-3 flex flex-col justify-start items-start gap-1">
            <div className="flex flex-row justify-start items-center gap-1 md:gap-2">
              <img src={thumbnail} alt="" className="w-6 md:w-auto" />
              <h3 className="font-patrick-hand text-[10px] md:text-3xl font-normal capitalize">
                {title}
              </h3>
            </div>
            <p className=" font-patrick-hand text-[8px] md:text-base">{description}</p>
          </div>
        </div>
      </div>
    </div>
